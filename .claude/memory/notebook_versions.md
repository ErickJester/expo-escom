...
**v4.8.0:** Option [3] nested step upgraded from a yes/no recursive prompt to a NUMBERED submenu (like [1]'s index). When the chosen subfolder has its own subfolders, shows: `[0]` TODO (whole subfolder = loose .jpg + all subfolders, recursive), `[1..n]` each nested subfolder (copy ONLY that one, recursive), and `[n+1]` TODAS (all nested subfolders). User wanted to pick ONE nested subfolder specifically (their case: «hora aventura» containing `adventure_time` + `dataset_descomprimido`). "todas" is auto-numbered (max+1), NOT hardcoded 9. `opcion_aplanar` now builds `fuentes` (list of (name,id)); each source goes through `copiar_contenido_recursivo`, results summed. Non-nested path still `copiar_contenido` (direct, with carpetas/otros counts).

**v4.7.0:** Option [3] (`opcion_aplanar`) now handles deeply nested folders. After picking the subfolder, if it has its own subfolders, asks whether to gather the WHOLE tree flattened into the PARENT folder («la padre de todos», e.g. «hora de aventura» inside dataset) — fixes the case where images trapped in sub-subfolders never reached the parent (old [3] copied only direct children and skipped folders). New `copiar_contenido_recursivo` (DFS stack walk collecting all .jpg at any depth, skip-by-name vs destino for re-run safety, parallel copy w/ NUM_COPY_WORKERS). Step-1 prompt reworded to "¿En qué carpeta PADRE debe caer todo?". Non-recursive path unchanged (still `copiar_contenido`).
